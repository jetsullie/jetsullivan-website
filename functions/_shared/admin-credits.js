...
export const createAdminCreditsHandlers = (storageKey) => ({
	onRequestGet: async ({ env }) => {
		try {
			const kv = requireContentKv(env);
			const document = await readCreditsDocument(kv, storageKey);
			return noStoreJson({
	...
	onRequestPost: async ({ request, env }) => {
		try {
			...
			const updatedDocument = await writeCreditsDocument(
				kv,
	...
	onRequestPut: async ({ request, env }) => {
		try {
			const kv = requireContentKv(env);
			const payload = await readCreditPayload(request);
	...
	onRequestPatch: async ({ request, env }) => {
		try {
			const kv = requireContentKv(env);
			const payload = await readCreditPayload(request);
	...
	onRequestDelete: async ({ request, env }) => {
		try {
			const kv = requireContentKv(env);
